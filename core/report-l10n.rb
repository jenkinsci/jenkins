...
list = Set.new
Dir.glob("src/**/*.properties").each do |f|
  f = File.basename(f)
  if f =~ /_(.._..)\.properties$/
    list << $1
  elsif f =~ /_(..)\.properties$/
    list << $1
  end
end

# from https://github.com/ged/linguistics/blob/master/lib/linguistics/iso639.rb
module ISO639
...
list = list.map do |l|
  ...
  e = ISO639::LANGUAGE_CODES[p.to_sym]
  if e then
    e[:eng_name]+(v ? " (#{v})" : "")
  else
    v
...
list.sort.each do |x| puts x end
