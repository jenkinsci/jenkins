        ...
        try {
            ...
            for (Entry<Computer, Integer> e : footprint.entrySet()) {
                ...
                for (LoadPredictor lp : LoadPredictor.all()) {
                    StreamSupport.stream(lp.predict(worksheet, computer, slot.start, slot.end).spliterator(),false).
                        limit(100).
                        forEach(fl -> timeline.insert(fl.startTime, fl.startTime+fl.duration, fl.numExecutors));
                }
