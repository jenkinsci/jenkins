 ...
 * @since 1.560
 */
public class EncryptedAgentJnlpFile implements HttpResponse {

    private static final Logger LOG = Logger.getLogger(EncryptedAgentJnlpFile.class.getName());
    private static final SecureRandom RANDOM = new SecureRandom();

    /**
     * The object that owns the Jelly view that renders JNLP file.
     * This is typically a {@link AgentComputer} and if so we'll use {@link AgentComputer#getJnlpMac()}
    ...
    public EncryptedAgentJnlpFile(AccessControlled it, String viewName, String agentName, Permission connectPermission) {
        this.it = it;
    ...
    public void generateResponse(StaplerRequest2 req, final StaplerResponse2 res, Object node) throws IOException, ServletException {
        ...
        if ("true".equals(req.getParameter("encrypt"))) {
            ...
            byte[] jnlpMac;
            if (it instanceof AgentComputer) {
                jnlpMac = Util.fromHexString(((AgentComputer) it).getJnlpMac());
            } else {
                jnlpMac = JnlpAgentReceiver.SLAVE_SECRET.mac(agentName.getBytes(StandardCharsets.UTF_8));
            }
