...
public class FilePathGlobalSettingsProvider extends GlobalSettingsProvider {
    ...
    public FilePath supplySettings(Run<?, ?> run, FilePath workspace, TaskListener listener) throws IOException, InterruptedException {
        ...
        try {
            ...
            LOGGER.log(Level.FINE, "Supply Maven global settings.xml file {0} to {1}", new Object[]{result, run});
            return result;
        } catch (Exception e) {
            throw new IllegalStateException(
                    "Failed to prepare Maven global settings.xml with path '" + path + "' for " + run +
