...
options {
  tokenVocab = CrontabLexer;
  superClass = BaseParser;
}

startRule [CronTab table] 
 : expr[0] { $table.bits[0]=$expr.ctx.bits; } WS expr[1] { $table.bits[1]=$expr.ctx.bits; } WS expr[2] { $table.bits[2]=$expr.ctx.bits; } WS expr[3] { $table.bits[3]=$expr.ctx.bits; } WS expr[4] { $table.dayOfWeek=(int)$expr.ctx.bits; } EOF
 | ( AT
       (
         'yearly'
         {
...
locals [int d=NO_STEP, int s, int e, int t]
   ...
   | STAR ( DIV token { $d=$token.ctx.value; } )?
   {
     $bits = doRange($d,$field);
   }
   | 'H' LPAREN token { $s=$token.ctx.value; } MINUS token { $e=$token.ctx.value; } RPAREN ( DIV token { $d=$token.ctx.value; } )?
   {
     $bits = doHash($s,$e,$d,$field);
   }
   | 'H' ( DIV token { $d=$token.ctx.value; })?
   {
     $bits = doHash($d,$field);
   }
   ;
