  ...
  function (e) {
      ...
      renderOnDemand(
        ...
        function () {
          function findInsertionPoint() {
            ...
            function findBestPosition(prospect, current, order) {
              function desirability(pos) {
                ...
                for (var i = 0; i < current.length; i++) {
                  if (i < pos == order(current[i]) <= order(prospect)) count++;
                }
                return count;
              }
            ...
            function o(did) {
              if (Object.isElement(did)) did = did.getAttribute("descriptorId");
              for (var i = 0; i < templates.length; i++)
                if (templates[i].descriptorId == did) return i;
              return 0; // can't happen
            }

            var bestPos = findBestPosition(t.descriptorId, current, o);
            if (bestPos < current.length) return current[bestPos];
    ...
    menuButton.getMenu().renderEvent.subscribe(function () {
      ...
      for (i = 0; i < items.length; i++) {
        ...
        if (t != null) applyTooltip(items[i].element, t);
      }
    });
