var breadcrumbs = (function () {
  ...
  var menuSelector = (function () {
    ...
    menuSelector.show = function (target) {
      ...
      if ($(target).hasClassName("inside")) xy[0] -= this.offsetWidth; // show the menu selector inside the text
      ...
      var updateParentSelector = menuSelectorTarget.getAttribute(
        "update-parent-class"
      );
  ...
  function invokeContextMenu(e, contextMenuUrl) {
    ...
    if (xhr) xhr.options.onComplete = function () {}; // ignore the currently pending call
