    ...
    if (isPageVisible()) {
        new Ajax.Request(ajaxUrl, {
            ...
            onSuccess: function (rsp) {
                ...
                const pivot = rows[firstBuildRow]
...
function checkRowCellOverflows(row) {
    ...
    function blockUnwrap(element) {
        const wrapped = $(element).getElementsBySelector('.wrapped')
    ...
    const buildName = $(row).getElementsBySelector('.build-name')[0]
    const buildDetails = $(row).getElementsBySelector('.build-details')[0]

    if (!buildName || !buildDetails) {
        return
    }

    const buildControls = $(row).getElementsBySelector('.build-controls')[0]
    let desc

    const descElements = $(row).getElementsBySelector('.desc')
    ...
    function resetCellOverflows() {
        ...
        // undo block wraps
        const blockWraps = $(row).getElementsBySelector('.block.wrap')
        for (let i = 0; i < blockWraps.length; i++) {
    ...
    const rowWidth = buildHistoryContainer.clientWidth
    const usableRowWidth = rowWidth - (leftRightPadding * 2)
    let nameOverflowParams = getElementOverflowParams(buildName)
    let detailsOverflowParams = getElementOverflowParams(buildDetails)

    let controlsOverflowParams
    if (buildControls) {
    ...
    function setBuildControlWidths() {
        if (buildControls) {
            ...
            if (buildBadge) {
                const buildControlsWidth = buildControls.clientWidth
                let buildBadgeWidth

                const buildStop = $(buildControls).getElementsBySelector('.build-stop')[0]
                if (buildStop) {
    ...
    let controlsRepositioned = false
    ...
    if (nameOverflowParams.isOverflowed || detailsOverflowParams.isOverflowed) {
        ...
        if (buildControls) {
            ...
            let badgesOverflowing = false
            let nameLessThanHalf = true
            let detailsLessThanHalf = true
            ...
            if ((!badgesOverflowing || nameLessThanHalf) &&
                ...
                wrap = blockWrap(buildName, buildControls)
