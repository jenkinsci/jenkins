...
$(function () {
  tabBarWidget.addPageTabs(".config-table.tabbed", function (tabBar) {
    ...
    if (tabBar.hasSections()) {
      var tabBarLastSectionKey =
        "config:" + tabBar.configForm.attr("name") + ":last-tab";
      var tabBarLastSection = jenkinsLocalStorage.getPageItem(
        tabBarLastSectionKey,
        tabBar.sections[0].id
      );
