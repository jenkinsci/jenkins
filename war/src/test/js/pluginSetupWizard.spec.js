...
function registerHandlebars() {
  ...
  Handlebars.registerHelper(
    "id",
    require("../../main/js/handlebars-helpers/id").default
  );
}
...
var ajaxMocks = function (responseMappings) {
    ...
    "/jenkins/setupWizard/platformPluginList": {
      ...
      data: [
        ...
        },
        {
          ...
          plugins: [{ name: "git" }, { name: "subversion", suggested: true }],
        },
        {
        ...
        },
        {
          ...
          plugins: [{ name: "email-ext" }, { name: "ssh" }],
        },
      ],
    },
  };
  ...
  return function (call) {
    ...
    var response =
      responseMappings[call.url] || responseMappings[`/jenkins${call.url}`];
    if (!response) {
      response =
        defaultMappings[call.url] || defaultMappings[`/jenkins${call.url}`];
    }
    if (!response) {
...
var validatePlugins = function (plugins, complete) {
  ...
  jenkins.post = function (url, data, cb) {
    ...
    if (!allMatch) {
      expect(JSON.stringify(plugins)).toBe(
        "In: " + JSON.stringify(data.plugins)
      );
    }
    // return status
