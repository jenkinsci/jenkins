                ...
                stage("${buildType} Build / Test") {
                    timeout(time: 180, unit: 'MINUTES') {
                        ...
                        withMavenEnv(["JAVA_OPTS=-Xmx1536m -Xms512m",
                            ...
                            def mvnCmd = "mvn -Pdebug -U -Dset.changelist help:evaluate -Dexpression=changelist -Doutput=$changelistF clean install -Dtest=DisablePluginCommandTest ${runTests ? '-Dmaven.test.failure.ignore' : '-DskipTests'} -V -B -Dmaven.repo.local=$m2repo -s settings-azure.xml -e"
